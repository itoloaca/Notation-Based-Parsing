...
def getContentMathML : HLet  = new HLet {
      ...
      payload foreach (p => {
        ...
        if (key != "position") {
          ...
          p._2 foreach (pos => {
            ...
            val start = pos(0)
            val length = pos(1)
            val value = input.substring(notStart + start, notStart + start + length)
      ...
      data += ("CML" -> result.toCML.toString)
      data += ("input" -> input)
//      Ok(Json.toJson(response.toMap))
      val resp = new JSONArray(List(result.toCML.toString))
      println("Sending Content Math ML respone")
			Server.JsonResponse(resp).aact(tk)
		}
	}
